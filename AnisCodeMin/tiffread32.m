...
% read tiff image files (including lsm)
...
% ------------------------------------------------------------------------------
%
% If you would like to acknowledge tiffread.m in a publication, 
...
fn='TIFFREAD32';	%for warning/error messages
        ...
        switch entry_tag
            ...
            case 262         % photometric interpretation
                ...
                if ( TIF.PhotometricInterpretation == 3 )
                    ...
                    disp([fn ': Warning:Ignoring TIFF look-up table']);
                end
            case 269
    ...
    if isfield(TIF, 'MM_stack')
        ...
        try
            ...
            try
                ...
                stack = cat(2, stack, IMG);
                fprintf('%s had to discard some meta_data\n', fn);
            catch
                fprintf('%s could not load dissimilar image %i\n', fn, img_indx);
            end
        end
...
%% ===========================================================================
    ...
    function plane = read_plane(offset, width, height, plane_nb)
        ...
        if ~all(size(plane) == [width height]),
            ...
            disp([fn ': Warning: Cropping data: found more bytes than needed']);
        end
        ...
        if strcmp(tok, 'Exposure')
            ...
            switch( unit )
                ...
                otherwise
                    ...
                    disp([fn ': MetaMorphExposure: Exposure unit not recognized']);
                    mm.Exposure = v;
            end
        else
            switch tok
                ...
                otherwise
                    try
                        ...
                        disp([fn ': MetaMorph: Invalid field']);
                    end
            end
        end
    end
    ...
    function R = readLSMinfo()
        ...
        if ( S.OffsetTimeStamps > 0 )
            ...
            if status == -1
                ...
                disp([fn ': Warning: Could not locate LSM TimeStamps']);
                return;
            end
