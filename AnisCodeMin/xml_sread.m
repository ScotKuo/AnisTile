function [tree, RootName, DOMnode] = xml_sread(xmlstring, Pref)
%xml_sread reads an xmlstring and converts it into Matlab's struct tree.
%	>>SCK 15_0130 Modified xml_read from xml_io_tools_2010_11_05.zip and from
%		XMLREADSTRING (MathWorks Support Team:
...
%
% DESCRIPTION
% tree = xml_sread(xmlstring) reads 'xmlstring' into data structure 'tree'
%
% tree = xml_sread(xmlstring, Pref) reads 'xmlstring' into data structure 'tree'
% according to your preferences
%
% [tree, RootName, DOMnode] = xml_sread(xmlstring) get additional information
% about XML file
%
% INPUT:
%  xmlstring	URL or filename of xml file to read
%  Pref     Preferences:
...
% OUTPUT:
%  tree         tree of structs and/or cell arrays corresponding to xmlstring
...
%
% DETAILS:
% Function xml_sread first uses XMLREADSTRING I found online (calls Java) and than
...
%???xmlstring=fread('text.xml');???
%   [tree treeName] = xml_sread (xmlstring);
%   disp(treeName)
%   gen_object_display()
...
%SCK Modified: call XMLREADSTRING instead of xmlread from file; returns a
%  JAVA structure, and the rest of this converts it to MatLab structure
if isempty(DOMnode), tree=[]; return; end
...
if (Debug)   % in debuging mode crashes are allowed
  ...
  try
    try
      ...
      error('Unable to parse XML string \n %s.', getReport(ME));
    end
  catch %#ok<CTCH> catch for mablab versions prior to 7.5
    error('Unable to parse XML string.',xmlstring);
  end
end
