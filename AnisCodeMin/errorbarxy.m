...
%                                p     pentagram
%                                h     hexagram
% SK modification: S has two more elements:
%	S{4}=UseAbsTicks (=0 for relative TickLen, =1 for absTickLen; def=0)
%	S{5}=[xtick ytick] (fxn of errBar for relative TickLen, else length of
%		tick for absTickLen; def=[0.1 0.1] as in original code).
% 
%   
...
%	SK 15_1019	Added extra S args for controlling tick length; commented
%					final 'hold off'.

%% SK globals (15_1019)
UseAbsTicks=0; %flag
xtickfxn=0.1; ytickfxn=0.1; %if UseAbsTicks=0
xticklen=0.25; yticklen=0.25; %if UseAbsTicks=1
...
if iscell(varargin{end}) & length(varargin{end})>=3 % search for formatting strings
	...
	if length(color)==3, color={color{:} UseAbsTicks, [xtickfxn ytickfxn]};
	elseif length(color)==4, color={color{:} [xtickfxn ytickfxn]}; end 
else
    color={'b', 'r', 'r', UseAbsTicks, [xtickfxn ytickfxn]};
end
UseAbsTicks=color{4}; xt=color{5};
if UseAbsTicks, xticklen=xt(1); yticklen=xt(2);
else xtickfxn=xt(1); ytickfxn=xt(2); end
if length(varargin)==4 % using errorbarxy(x, y, errx, erry)
    errx=varargin{3}; erry=varargin{4};
    if ~isempty(errx)
        lx=x-errx; ux=x+errx;
    else
        lx=[]; ux=[];
...
elseif length(varargin)==6 % using errorbarxy(x, y, lerrx, uerrx, lerry, uerry)
    lx=x-varargin{3}; ux=x+varargin{4};
    ly=y-varargin{5}; uy=y+varargin{6};
    if ~isempty(lx)
        errx=(ux-lx)/2;
    else
        errx=[];
...
for k=1:length(x)
	xt=xticklen; yt=yticklen;
    if ~isempty(lx) & ~isempty(ly) % both errors are specified
        l1=line([lx(k) ux(k)],[y(k) y(k)]);
        hold on;
		if ~UseAbsTicks, yt=ytickfxn*erry(k); xt=xtickfxn*errx(k); end
        l2=line([lx(k) lx(k)],[y(k)-yt y(k)+yt]);
        l3=line([ux(k) ux(k)],[y(k)-yt y(k)+yt]);
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        l5=line([x(k)-xt x(k)+xt],[ly(k) ly(k)]);
        l6=line([x(k)-xt x(k)+xt],[uy(k) uy(k)]);
        allh(k, :)=[l1, l2, l3, l4, l5, l6];
    elseif isempty(lx) & ~isempty(ly) % x errors are not specified
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        hold on;
        errx=nanmean(abs(diff(x)));
		if ~UseAbsTicks, xt=xtickfxn*errx(k); end
        l5=line([x(k)-xt x(k)+xt],[ly(k) ly(k)]);
        l6=line([x(k)-xt x(k)+xt],[uy(k) uy(k)]);
        allh(k, 4:6)=[l4, l5, l6];
    elseif ~isempty(lx) & isempty(ly) % y errors are not specified
		...
		if ~UseAbsTicks, yt=ytickfxn*erry(k); end
        l2=line([lx(k) lx(k)],[y(k)-yt y(k)+yt]);
        l3=line([ux(k) ux(k)],[y(k)-yt y(k)+yt]);
        allh(k, 1:3)=[l1, l2, l3];
