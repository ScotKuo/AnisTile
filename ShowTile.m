...
if replaceFig, close all; end % close figs (for cbfreeze); letter size aspect ratio
bigfigure([],1);
loc.nc=5;
if ~saveFig, loc.nr=numel(imgs.im);
	v2=get(gcf,'position'); vh=v2(3)/loc.nc; v4=loc.nr*vh+25;
	v2(2)=v2(4)-v4; v2(4)=v4; set(gcf,'position',v2);
else loc.nr=4; end %prettier PDF if 4-color
loc=getLoc(loc); %must have a figure window already open to execute properly
ns=numel(setnum);
	%max range of data
...
for j=1:ns,
	if j>1,%repeat code above, but getLoc already ran
		if replaceFig, close all; end
		bigfigure([],1);
		if ~saveFig, v2=get(gcf,'position'); vh=v2(3)/loc.nc; v4=loc.nr*vh+25;
		v2(2)=v2(4)-v4; v2(4)=v4; set(gcf,'position',v2); end
	end
	...
	for i=1:numel(imgs.im), %channel
		if i==1, loc.firstRow=1; else loc.firstRow=0; end
...
% freezeColors; %don't freezeColors for 'surface'/pcolor: problems for eps export
	...
	pn=pn+1; %subplot(loc.nr,loc.nc,loc.base+pn);
	if ~plotTight, subplot(loc.nr,loc.nc,loc.base+pn); %ang histogram
		else subplot('position',[xb yb tightw tighth]); end
		histshow=35; histbin=10; histlim=175; %force zero-centered histogram with specified binwidth
		histlim=ceil((histlim-histbin/2)/histbin)*histbin+histbin/2;
		histogram(mean(rang(:))-rang(:),[-histlim:histbin:histlim],'Normalization','probability');
		xlim(histshow*[-1 1]); ylim([0 0.55]);
		if plotTight, set(gca,'XTick',[]); set(gca,'XTickLabel',{});
			set(gca,'YTick',[]); set(gca,'YTickLabel',{});
			set(gca,'Position',[xb yb tightw tighth]);
			set(gca,'box','on'); %set(gca,'color','none');
			xb=xb+tightw; end
		if plotTight && firstRow, title(['AngDistr(\pm' num2str(histshow) ')']); end
        
    pn=pn+1; %subplot(loc.nr,loc.nc,loc.base+pn);
    ranis=acGrid.anis;
	if ~plotTight, subplot(loc.nr,loc.nc,loc.base+pn); %anis histogram
		else subplot('position',[xb yb tightw tighth]); end
		histshow=5; histbin=0.5; histlim=10; %force zero-centered histogram with specified binwidth
		histlim=ceil((histlim-histbin/2)/histbin)*histbin+histbin/2;
		histogram(ranis(:),[0:histbin:histlim],'Normalization','probability');
		xlim([1,histshow]); ylim([0 0.35]);
		if plotTight, set(gca,'XTick',[]); set(gca,'XTickLabel',{});
			set(gca,'YTick',[]); set(gca,'YTickLabel',{});
			set(gca,'Position',[xb yb tightw tighth]);
			set(gca,'box','on'); %set(gca,'color','none');
			xb=xb+tightw; end
		if plotTight && firstRow, title(['AnisDistr(1-' num2str(histshow) ')']); end

	pn=pn+1; %subplot(loc.nr,loc.nc,loc.base+pn);
	if ~plotTight, subplot(loc.nr,loc.nc,loc.base+pn); %rose plot
